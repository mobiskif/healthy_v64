...
class Hub {
    ...
    private fun readSOAP(body: String, action: String): XmlPullParser? {
        try {
            ...
            Log.e("jop", "Ошибка чтения SOAP " + e.toString())
            return null
        }
    ...
    fun GetPat(action: String, args: Array<String>): MutableMap<String, String> {
        ...
        try {
            ...
            set = mutableMapOf()
            set["IdPat"] = ""
            set["ErrorDescription"] = "Карточка пациента не найдена. Вводимые ФИО, дата рождения и данные в регистратуре выбранной поликлиники должны совпадать. Позвоните/сходите в регистратуру для сверки"
            return set
        }
